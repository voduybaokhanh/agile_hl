...
router.get('/list', async (req, res) => {
  try {
    ...
    res.json({ status: false, message: 'Lỗi khi lấy danh sách phim: ' + err.message });
  }
});
...
router.post('/add', async (req, res) => {
  try {
    const { tenPhim, moTa, thoiLuong, theLoai, ngayPhatHanh, trailer, admin } = req.body;
    ...
    // Tạo phim mới
    const newPhim = new phimRouter({ tenPhim, moTa, thoiLuong, theLoai, ngayPhatHanh, trailer, admin });
    await newPhim.save();
...
router.put('/update', async (req, res) => {
  try {
    const { id, tenPhim, moTa, thoiLuong, theLoai, ngayPhatHanh, trailer } = req.body;
    if (!id) {
    ...
    res.json({ status: true, message: 'Cập nhật phim thành công', data: updatedPhim });
  } catch (err) {
...
router.get('/detail', async (req, res) => {
  try {
    ...
    // Tìm phim theo ID
    const phim = await phimRouter.findById(id).populate('admin', 'ten email');
    if (!phim) {
      return res.json({ status: false, message: 'Không tìm thấy phim' });
    }
    res.json({ status: true, data: phim });
  } catch (err) {
    res.json({ status: false, message: 'Lỗi lấy chi tiết phim: ' + err.message });
  }
});
